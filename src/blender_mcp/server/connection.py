...
class _Connectable(Protocol):
    """Structural type for the one thing `connect_with_retry` drives."""
...
def connect_with_retry(
    connection: _Connectable, *, attempts: int, delay: float, sleep: Callable[[float], None] | None = None
) -> bool:
    """
...
def get_blender_connection():
    ...
    with _connection_lock:
        ...
        # Retried, not a bare connect(): a Blender still enabling its addon, or a container that
        # has started without having bound its port yet, is a short wait rather than a failed
        # first tool call. BLENDER_CONNECT_ATTEMPTS=1 restores the single attempt. The lazy
