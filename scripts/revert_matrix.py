...
NEW_NODES_IN_EXISTING_FILES = (
    ...
    *(f"{AMT}::{LIST_SCALAR}[{field}]" for field in ("capabilities", "writable_output_roots")),
    ...
    # --- create_studio_lighting refuses a doomed preview before it builds the rig ---
    f"{LIGHTT}::test_create_studio_lighting_refuses_a_bad_preview_before_building_the_rig[relative-path]",
    f"{LIGHTT}::test_create_studio_lighting_refuses_a_bad_preview_before_building_the_rig[not-png]",
    f"{LIGHTT}::test_create_studio_lighting_refuses_a_bad_preview_before_building_the_rig[unconfirmed-cycles-samples]",
    f"{LIGHTT}::test_create_studio_lighting_refuses_a_bad_preview_before_building_the_rig[frame-beyond-blender]",
...
NOT_INDIVIDUALLY_FALSIFIABLE: dict[str, str] = {
    ...
    f"{STRICTT}::test_a_valid_call_still_validates_through_its_nested_model": (
        "`extra='forbid'` and `allow_inf_nan=False` only ever add a refusal, so no revert of the "
        "hardening can stop a well-formed call validating. Measured, not argued: with "
        "`harden_tool_arguments(mcp)` neutralised the file reports `10 failed, 2 passed` and this is "
        "one of the two, because the nested `CameraOpticsPatch` is generated by the SDK from the "
        "handler's signature and owes nothing to the config `_strict_args` writes. It is the control "
        "for its siblings: it exists to catch a hardening pass that rebuilt the top-level model badly "
        "enough to lose the nested patch type, which is a change to `_strict_args`, not a revert of it."
    ),
