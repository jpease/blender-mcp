...
# provenance block, and `helpers` reads the add-on id. The real `__init__` would register
# Blender classes, so only those constants are provided, with fixed values so a version bump
# cannot move this transcript.
