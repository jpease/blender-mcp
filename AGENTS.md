...
- Run the relevant test suite and quality checks before handing off a change.
  ...
  findings already in it. Function-size metrics (`PLR0912`/`PLR0914`/`PLR0915`,
  nesting depth) are ratcheted per function against the base: editing inside a
  legacy oversized function is fine, raising its measured value is not.
  basedpyright has no backlog left, so it is enforced across the whole tree;
  there is nothing to ratchet and a line filter would only weaken it.
  ...
  fake `bpy`. `just smoke` runs every `tests/blender_*_smoke.py` script against
  a real headless Blender in well under a minute and is the only thing that
...
and production workflow coverage. **That count is historical**: the surface has grown several times
over since, and a server process registers only a subset of it.

Read the count as "what this audit covered", never as "what a session sees". Three other numbers
are routinely mistaken for each other and for this one; none is written down here because each
moves with every tool added, so measure it instead:

- **The whole catalog** — every tool, registered only with `BLENDER_MCP_TOOLSETS=all`:
  `just catalog all` prints `tools`.
- **The default `core` bundle** a process registers when `BLENDER_MCP_TOOLSETS` is unset: scene
  inspection, object editing, viewport, animation, file lifecycle/linking (`just catalog`).
  Camera, rendering, lighting, world, character posing, texture, retopology and the simulation
  domains are **absent by design**, not missing. `shot` (`just catalog shot`) adds camera,
  lighting, rendering and character posing; see the Tool Bundles table in `README.md`.
- **`get_addon_status`'s `capability_count`** — Blender-side socket commands the add-on
  dispatches (`just addon-surface` prints the count it snapshots), which is a different surface
  from the MCP tools a client mounts. A full `capability_count` alongside a short tool list is the
  expected shape, not a registration fault.
...
| No atomic transaction or rollback contract | **Solved** | `bundled/addon/transaction.py` with `object_state.py` gives one explicit checkpoint per mutating request, identity-based tracking of created datablocks and their removal on failure, covered by 21 tests in `tests/test_mutation_transaction.py`. Every mutating command is dispatched inside it (`server_core.py:854-872`), and a handler that *returns* a failure shape is converted to an error inside the transaction so its partial mutation rolls back (`:897-902`). Two limits remain (`mutation_transaction`'s docstring lists the restored fields and the exclusions): collection membership is not a restored field, and a deleted pre-existing datablock is never resurrected, so `mesh_boolean(keep_cutter=False)` and `reset_scene` are not recoverable by rollback part-way. |
...
| Inspection coordinate and evaluation contracts | **Solved** | `get_object_info` documents local transform vs world-space AABB and how to read `rotation` by `rotation_mode` (`server/tools/viewport.py:93-114`), and returns native rotation and modifiers (`bundled/addon/handlers/scene_inspection.py:212-251`); `get_mesh_data` documents base-mesh local coordinates (`server/tools/viewport.py:145-149`). |
...
| Socket size cap is incomplete | **Solved** | `extract_frames` drops the connection for an oversized frame after splitting and for an oversized terminator-less remainder (`bundled/addon/socket_transport.py:25-70`); responses over the cap become error frames (`:409-410`). |
| `create_studio_lighting` validates after it mutates, and blocks its own retry | **Solved** | The tool dispatched the rig and only then ran the mandatory preview, whose argument checks lived in `render_lighting_preview`, so a bad `preview_output_path` or unconfirmed Cycles samples above 64 raised after three lights existed, and the handler refused the corrected retry's member names (`handlers/lighting/construction.py:419-424`). `validate_preview_arguments` (`server/tools/lighting/rendering.py:149`) is now the one preflight both tools pass - frame range (`:186`), Cycles sample gate (`:188`), path arguments fitting the engine, absolute `.png`, distinct paths - and `create_studio_lighting` calls it before dispatching the rig (`server/tools/lighting/construction.py:257`), `render_lighting_preview` before resolving its temporary paths (`rendering.py:254-258`). What only Blender can refuse - a missing output directory, an existing file without `confirm_overwrite`, an unavailable engine - still comes from the preview handler after the rig exists: the server cannot see Blender's filesystem when Blender runs on another host. The handler's own validate-then-rollback is unchanged. `tests/server/tools/lighting/test_tools.py::test_create_studio_lighting_refuses_a_bad_preview_before_building_the_rig`. |
| Non-finite vectors reach core handlers | **Solved** | `StrictModel` refused NaN/inf only inside payload models; a top-level `float`, tuple or list of floats was validated by the SDK-generated argument model under pydantic's `allow_inf_nan=True`, so `create_primitive_object(location=[nan, 0, 0])` reached `handlers/mesh.py`. The hardening pass that already forbade unknown keys now gives every registered tool's argument model `StrictModel`'s whole config (`server/tools/_strict_args.py:80`, run at `server/tools/__init__.py:23`), so every top-level float argument - 253 across 114 of the 305 tools, bare, optional, tuple or list - refuses NaN, inf and -inf before dispatch, and the two configs cannot drift apart. `allow_inf_nan` is absent from JSON schema, so the advertised catalog is byte-identical; no handler changed. `tests/server/test_strict_tool_args.py::test_every_float_argument_in_the_catalog_refuses_a_non_finite_value` sweeps the catalog; `::test_create_primitive_object_refuses_a_non_finite_vector_before_dispatch` runs the reported case through the registered tool. |
| Geometry-nodes builders ignore every referenced object's world transform | **Solved** | Every Object/Collection Info node is now made by `_source_node` (`handlers/geometry_nodes/workflows.py:118-140`), which has no default `transform_space` and sets it on the node (`:138`), so each site states which reading it means. RELATIVE - where the source sits, in the host's local space - for what a builder promises to follow: the boolean's cutter object or collection (`:1263`), the radial pivot (`:1001`, whose Location output is then the pivot's world position in host space), the CURVE-layout path (`:1042`), the curve generator's path (`:681`) and the proximity-push target (`:1416`). ORIGINAL - the source's own shape, wherever it sits - for what the graph places itself: scatter and array instances (`:518`, `:1062`), panel collections (`:1157`) and the curve generator's profile (`:770`; read RELATIVE, the profile's distance from the host would offset the tube off its path). The curve generator's `coordinate_space` reply and the builders' tool docstrings (`server/tools/geometry_nodes/workflows.py`) now say this. `tests/blender_geometry_nodes_transform_smoke.py` measures each in the evaluated depsgraph with hosts away from the origin: a cutter over the target cuts it and one moved 9 units away leaves 8 verts; the radial centroid is the pivot's world location; a CURVE array and a curve generator follow a moved curve; a proximity target moved away stops pushing. Each assertion was falsified by reverting its site. `tests/test_geometry_nodes_builder_spaces.py` holds the per-site choice in CI with a fake graph. |
| `create_curve_generator`'s `radius` is inert on Blender 5.2 | **Solved** | A Radius input node now feeds Curve to Mesh's `Scale` field (`handlers/geometry_nodes/workflows.py:236-237`, in `_curve_to_mesh`), so the exposed Radius control, which sets the curve radius, scales the profile again. `tests/blender_geometry_nodes_transform_smoke.py` measures a 0.1-thick tube at radius 0.05 and a 1.0-thick one at 0.5 (both were 2.0 before); `tests/test_geometry_nodes_builder_spaces.py::test_curve_generator_scales_its_profile_by_the_curve_radius` holds the link in CI. |
| Disabled integrations remain visible to agents | **Solved** | Mounting still follows `BLENDER_MCP_TOOLSETS`; usability now follows the handshake. `server/integrations.py` maps each integration to its tools module, gating command and sidebar checkbox. `BlenderFastMCP` (`server/app.py:168`) omits a disabled integration's tools from tools/list (`:219-225`), refuses a call to one before dispatch with the checkbox to tick (`:227-236`; a cached 'disabled' is confirmed by a fresh handshake first, `integrations.py:143`), and sends notifications/tools/list_changed to a session that listed tools once a later handshake changes the set (`:238`), advertising `listChanged` at initialize (`:207`). Nothing is withheld before a handshake. `get_addon_status`'s `tool_lookup` names the checkbox for a withheld tool (`server/tools/core.py:138`). Covered through a real client session in `tests/server/test_integration_gating.py`. |

## Pending findings

| Priority | Finding | Status | Recommendation |
|---|---|---|---|
| Critical | Unauthenticated socket | **Pending** | Auto-start already defaults to off (`bundled/addon/__init__.py:52-55`, and `:105` when no scene is available), but a started server binds `localhost` (`bundled/addon/server_core.py:188`, `socket_transport.py:164`) and authenticates nothing (`server_core.py:603-604`), so any local process that reaches the port can send any add-on command, including `open_shot`, `save_shot`, `render_scene` and `relocate_library`. Authenticate each connection with a per-session secret in the handshake and refuse unauthenticated frames; loopback binding is not authentication. |
...
Relative to the comparable MCPs used in the original audit, this implementation now has a stronger structured modeling and inspection surface and no arbitrary-script path. Its remaining production gaps: an authenticated transport and provider downloads off Blender's main thread.
