...
| Command | Result | When |
|---|---|---|
| `just check` | `lint: clean` (194 changed files, 13,413 owned lines); `fmt-check: 448 files already formatted`; `typecheck: 0 errors, 0 warnings, 0 notes`; `test: 1812 passed, 4 skipped in 63s` (the skips are the live-GUI gates) | uncommitted, after the second rehearsal pass |
| `just smoke` | `ok` on all 28 `tests/blender_*_smoke.py` against real headless Blender 5.2.2, ~21s, including the batched pose keying and bone-`target_bone` aim in `blender_character_posing_smoke.py`, the library-override custom-property contract in `blender_character_rigging_phase0_smoke.py`, `ready: true` in `blender_scene_validate_smoke.py`, the period guard in `blender_cycle_period_smoke.py` and bone framing in `blender_camera_framing_smoke.py` | uncommitted, after the second rehearsal pass |
