...
- **The repo is mounted read-only** at `/repo`. The addon is copied out of it and the server runs from it on every start, so code edits need only a `docker compose restart`. Dependency changes (`poetry.lock`) need a `docker compose build`; the image installs exactly the locked versions. Tools that try to write into the project tree will fail.
