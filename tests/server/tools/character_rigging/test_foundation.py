...
into foundation.py so both tools share one contract instead of two.
"""
...
def _rig_with_bones(monkeypatch: pytest.MonkeyPatch, *names: str):
    ...
    foundation = sys.modules[f"{addon.__name__}.handlers.character_rigging.foundation"]
    return foundation, rig


def test_selected_bones_returns_every_bone_in_armature_order_when_unfiltered(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    foundation, rig = _rig_with_bones(monkeypatch, "root", "spine", "head")

    assert [bone.name for bone in foundation._selected_bones(rig, None)] == ["root", "spine", "head"]
...
def test_selected_bones_returns_named_bones_in_armature_order_not_request_order(
    ...
    foundation, rig = _rig_with_bones(monkeypatch, *(f"CHAR1_bone_{index:03d}" for index in range(180)))

    filtered = foundation._selected_bones(rig, ["CHAR1_bone_177", "CHAR1_bone_004"])
...
def test_selected_bones_refuses_an_unknown_name(monkeypatch: pytest.MonkeyPatch) -> None:
    foundation, rig = _rig_with_bones(monkeypatch, "head", "spine")

    with pytest.raises(ValueError, match=r"Bones not found in armature 'CHAR1_rig': \['hed'\]"):
        foundation._selected_bones(rig, ["head", "hed"])
